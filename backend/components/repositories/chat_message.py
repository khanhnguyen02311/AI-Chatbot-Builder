...
class ChatMessageRepository(BaseRepository, ABC):
    ...
    def get_all_by_chat_session(self, chat_session_identifier: int):
        query = select(PostgresModels.ChatMessage).filter(PostgresModels.ChatMessage.id_chat_session == chat_session_identifier).order_by(PostgresModels.ChatMessage.id.desc())
